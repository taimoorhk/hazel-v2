<template>
  <div class="fixed-stats-dashboard">
    <!-- Loading State -->
...
// Chart options with better responsiveness
const chartOptions = {
  responsive: true,
...
// Chart data computed properties with fallback data
...
const getHighEngagementCount = () => {
  if (!statsData.value?.canary_analysis_files) return 0
  const engagements = statsData.value.canary_analysis_files.map((file: any) => file.canary_data.analysis.engagement)
...
const loadStats = async () => {
  ...
  try {
    ...
    if (result.success && result.data) {
      ...
      console.log('📊 Loaded fixed stats data:', result.data)
    } else {
      throw new Error(result.message || 'Failed to load enhanced canary data')
    }
  } catch (err) {
    console.error('Error loading fixed stats:', err)
