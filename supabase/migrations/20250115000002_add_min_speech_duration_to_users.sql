-- Add min_speech_duration field to public.users table
-- This field controls the minimum duration of speech before it's considered valid
-- Range: 0.02 to 0.35 seconds with 0.01 second precision
-- Default: 0.05 seconds for all users

-- Add the new column with constraints
ALTER TABLE public.users 
ADD COLUMN min_speech_duration DECIMAL(3,2) NOT NULL DEFAULT 0.05 
CHECK (min_speech_duration >= 0.02 AND min_speech_duration <= 0.35);

-- Add a comment to document the field
COMMENT ON COLUMN public.users.min_speech_duration IS 'Minimum duration of speech before it is considered valid (0.02 to 0.35 seconds, 0.01 precision)';

-- Update existing users to have the default value
UPDATE public.users 
SET min_speech_duration = 0.05 
WHERE min_speech_duration IS NULL;

-- Create an index for better performance on queries involving this field
CREATE INDEX IF NOT EXISTS idx_users_min_speech_duration ON public.users(min_speech_duration);
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE NOTICE '✅ min_speech_duration column added successfully to public.users table';
    ELSE
        RAISE EXCEPTION '❌ Failed to add min_speech_duration column';
    END IF;
    
    -- Check if the constraint was applied
    IF EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name LIKE '%min_speech_duration%'
    ) THEN
    ...
    IF EXISTS (
        ...
        AND table_name = 'users' 
        AND column_name = 'min_speech_duration'
        AND column_default = '0.05'
    ) THEN
        RAISE NOTICE '✅ Default value 0.05 set successfully';
    ELSE
